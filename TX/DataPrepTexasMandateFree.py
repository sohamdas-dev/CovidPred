...
def dataPreparation_mandateFree(state_name, I2HospitalRate, E2Irate, I2Rrate, delay_from_hospital_to_I, N0, S0, n0,
    ...
    def calc_Beta_coeff(inpData):
        ...
        curr_Beta = (Beta_coeff_summer+Beta_coeff_winter)/2 + 0.5*(Beta_coeff_winter - Beta_coeff_summer)\
            * np.cos(2*math.pi*(inpData['day_count']-320)/365)
        curr_Beta = (Beta_coeff_summer+Beta_coeff_winter)/2 + 0.5*(Beta_coeff_winter - Beta_coeff_summer)
        # curr_Beta = (Beta_coeff_summer+Beta_coeff_winter)/2 + 0.5*(Beta_coeff_winter - Beta_coeff_summer)\
        #     * abs(np.cos(2*math.pi*(inpData['day_count']-28)/365))
        # return curr_Beta
    ...
    #
    inpData.to_csv('./Data/NEW-' + state_name + '-history-Final-Data-with-seasonality-and-sinusoidV2.csv')
    #
    return None

...
#
true_I = overall_result_for_plot.loc[(overall_result_for_plot.index <= indLastAvailData)]
forecasted_I = overall_result_for_plot.loc[(overall_result_for_plot.index > indLastAvailData)]
#
#
if False:
    true_I_new = true_I
    true_I_new.rename(columns = {'I':'True_I'}, inplace = True)
    forecasted_I_new = forecasted_I
    forecasted_I_new.rename(columns = {'I':'Forecasted_I'}, inplace = True)
    #
    fig1, ax1 = plt.subplots()
    sns.relplot(data=true_I_new, kind="line", ax=ax1)
    fig2, ax2 = plt.subplots()
    sns.color_palette("flare", as_cmap=True)
    sns.relplot(data=forecasted_I_new, kind="line", ax=ax2, color=".7")
#
5+6
#
# # works: v1: colors are uniform
# true_and_forecasted_I = true_I
...
if True:
    ...
    sns.relplot(data=true_and_forecasted_I, x="time", y = "I", hue="source", kind="line", ax=ax3)
#
#
